const homepage=`<div class="search-container">
    <img src="icon.png" alt="Search Icon" class="search-icon"  width="auto" height="auto">
...
<img src="icon.png" class="search-icon" alt="Search Icon" >
<input type="text" class="search-input" id="searchInput" placeholder="">
<button id="searchButton" class="search-button">Scloud Search</button>
</div>`
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  async function search(type = 0, text = "") {
    ...
    var resultsHtml = `<h2>Search Results</h2><div id="searchResults"></div>`;
    ...
    // Make the search results scrollable
    var scrollableDiv = document.createElement('div');
    scrollableDiv.className = 'scrollable'; // Add a class for styling
    scrollableDiv.appendChild(document.getElementById('searchResults'));

    document.getElementById("content").appendChild(scrollableDiv);
...
function fetchTermPages(term) {
  let termLinks=[];
  for (var i = 0; i < terms.length; i++) {//cancel the loop an d switch with button value
...
async function displayLinks(term){
  ...
  //console.log(links);
  // Example usage:
  //var items = ['Item 1', 'Item 2', 'Item 3', 'Item 4']; // Array of items
  //var targetDivId = 'targetDiv'; // ID of the target div where you want to load the list
  //createAndLoadList(items, targetDivId);
